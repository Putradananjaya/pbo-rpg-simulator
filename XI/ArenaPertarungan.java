        ...
        while (isBermain) {
            ...
            for (int i = 0; i < gelombangMonster.length; i++) {
                if (gelombangMonster[i].healthPoint > 0) {
                    ...
                    // Fitur dari tantangan minggu lalu tetap berjalan
                    monsterAktif.suaraKhas();

                    // --- IMPLEMENTASI CASTING ---
                    // Cek apakah monster ini memiliki kemampuan terbang
                    if (monsterAktif instanceof BisaTerbang) {
                        System.out.println("[PERINGATAN! SERANGAN UDARA TERDETEKSI]");

                        // Downcasting: Mengubah perspektif Java dari Musuh menjadi BisaTerbang
                        BisaTerbang monsterTerbang = (BisaTerbang) monsterAktif;

                        // Panggil metode khusus penerbangan
                        monsterTerbang.lepasLandas();
                        monsterTerbang.seranganUdara();
                    } else {
                        // Jika tidak bisa terbang (Slime dan Zombie), lakukan serangan darat
